...
impl Allocator {
    ...
    pub fn create_pool_with_ref(
        ...
        unsafe {
            ...
            ffi::vmaCreatePool(self.internal, &raw_info, &mut ffi_pool).into_result()?;
...
impl<A: Deref<Target = Allocator>> AllocatorPool<A> {
    ...
    pub fn get_statistics(&self) -> vk::Result<ffi::VmaStatistics> {
        unsafe {
    ...
    /// Retrieves statistics of existing `AllocatorPool` object.
    pub fn calculate_statistics(&self) -> vk::Result<ffi::VmaDetailedStatistics> {
        unsafe {
    ...
    pub fn check_corruption(&self) -> vk::Result<()> {
        unsafe { ffi::vmaCheckPoolCorruption(self.allocator.internal, self.pool).map_success(|| ()) }
    }

    /// Decomposes the `AllocatorPool` into a raw handle and a pointer to the allocator the pool is
    /// allocated from.
    ///
    /// Onwership of the pool is transferred to the caller.
    pub fn into_raw_parts(self) -> (RawPoolHandle, A) {
        let pool = ManuallyDrop::new(self);
        let p = pool.pool;
        let a = unsafe { (&raw const pool.allocator).read() };
        (p, a)
    }

    /// Imports an already existing allocator pool.
    ///
    /// # Safety
    ///
...
pub trait Alloc {
    ...
    unsafe fn create_buffer(
        ...
        buffer_info: &vk::BufferCreateInfo,
        create_info: &AllocationCreateInfo,
    ) -> vk::Result<(vk::raw::Buffer, Allocation)> {
        let mut create_info: ffi::VmaAllocationCreateInfo = create_info.into();
        create_info.pool = self.pool();
        ...
        Ok((buffer.assume_init(), Allocation(allocation)))
    }
    ...
    unsafe fn create_image(
        ...
        image_info: &vk::ImageCreateInfo,
        create_info: &AllocationCreateInfo,
    ) -> vk::Result<(vk::raw::Image, Allocation)> {
        let mut create_info: ffi::VmaAllocationCreateInfo = create_info.into();
        create_info.pool = self.pool();
        ...
        Ok((image.assume_init(), Allocation(allocation)))
    }
}
